...
async def runLogger(
    port: str, path: str, cancelToken: CancelToken | None = None
) -> None:
    file = open(path, "w")
    ...
    try:
        ...
        file.write(
            "time[ms],gyro x[rad/s],gyro y[rad/s],gyro z[rad/s], accel x[m/s^2], accel y[m/s^2],accel z[m/s^2],inv radius[1/m],direction[rad],\n"
        )
        ...
        while cancelToken is None or not cancelToken.triggered:
            ...
            file.write(
                f"{consecutiveTimestamp_ms},{imuData.gyroX},{imuData.gyroY},{imuData.gyroZ},{imuData.accelX},{imuData.accelY},{imuData.accelZ},{motionData.invRadius},{direction}\n"
            )
